...
export default function ExtractArchiveModal({ opened, closePath, path }: ExtractArchiveModalProps) {
  ...
  return (
    <Modal opened centered title="Extract Archive" onClose={() => navigate(closePath)}>
      <Text size="sm" mb="md">
        Extracting <strong>{fileName}</strong> will decompress its contents into the destination
        folder.
      </Text>
      <ValidatedForm
        method="post"
