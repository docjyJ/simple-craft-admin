...
describe("resolveSafePath", () => {
	...
	it("bad uid", () => {
		expect(() => resolveSafePath("abc_123", "/server.jar")).toThrow("Invalid uid: 'abc_123'. Only alphanumeric characters, and hyphens are allowed.");
		expect(() => resolveSafePath("abc/123", "/server.jar")).toThrow("Invalid uid: 'abc/123'. Only alphanumeric characters, and hyphens are allowed.");
	});
	it("out of root", () => {
		expect(() => resolveSafePath("abc-123", "../abc-123/server.jar")).toThrow("Path '../abc-123/server.jar' is outside the root directory.");
		expect(() => resolveSafePath("abc-123", "../../other-server/server.jar")).toThrow("Path '../../other-server/server.jar' is outside the root directory.");
		expect(() => resolveSafePath("abc-123", "/server/../data/../../server.jar")).toThrow("Path '/server/../data/../../server.jar' is outside the root directory.");
	});
});
